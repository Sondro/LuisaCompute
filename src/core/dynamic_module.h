...
private:
    ...
    DynamicModule(void *handle) : _handle(handle) {}

public:
    static std::optional<DynamicModule> load(const std::filesystem::path &folder, std::string_view name);
    static std::optional<DynamicModule> load(std::string_view name);
    /**
