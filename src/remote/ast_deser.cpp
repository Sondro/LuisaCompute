...
void AstSerializer::DeserFunction(detail::FunctionBuilder* builder, IJsonDict const* dict) {
	...
	auto GetMapVar = [&](vstd::string_view name, luisa::vector<Variable>& dst, IJsonDict*& sub) {
		sub = dict->Get(name).get_or<IJsonDict*>(nullptr);
		assert(sub);
		dst.reserve(sub->Length());
		for (auto&& i : *sub) {
			...
			assert(varDict);
			Variable& v = dst.emplace_back();
			v._uid = static_cast<uint64>(i.key.get_or<int64>(0));
			DeSerialize(v, varDict);
		}
	};
	//////////////////// Builtin vars
	...
	//////////////////// Constant
	{
		...
		for (auto&& i : *constDict) {
			...
			auto&& result = builder->_captured_constants.emplace_back();
			if (typeHash)
	...
	//////////////////// Atomic flag
	builder->_requires_atomic_float =  dict->Get("atomicfloat"sv).get_or<bool>(false);
}
...
void AstSerializer::DeserializeKernel(detail::FunctionBuilder* builder, IJsonDict const* dict) {
	...
	auto disposeStackAlloc = vstd::scope_exit([&] { stackAlloc.Delete(); });
