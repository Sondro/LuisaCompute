...
class SPV_API Optimizer {
 public:
  ...
  // spvstd::string of a form accepted by Optimizer::FlagHasValidForm().
  //
  ...
  bool RegisterPassesFromFlags(const spvstd::vector<spvstd::string>& flags);
  ...
  //                  HLSL front-end.
  bool RegisterPassFromFlag(const spvstd::string& flag);
  ...
  // returns false.
  bool FlagHasValidForm(const spvstd::string& flag) const;
  ...
  bool Run(const uint32_t* original_binary, size_t original_binary_size,
           spvstd::vector<uint32_t>* optimized_binary) const;
  ...
  bool Run(const uint32_t* original_binary, const size_t original_binary_size,
           spvstd::vector<uint32_t>* optimized_binary,
  ...
  // Returns a spvstd::vector of strings with all the pass names added to this
  // optimizer's pass manager. These strings are valid until the associated
  // pass manager is destroyed.
  spvstd::vector<const char*> GetPassNames() const;
...
Optimizer::PassToken SPV_API CreateEliminateDeadFunctionsPass();
...
Optimizer::PassToken SPV_API CreateEliminateDeadMembersPass();

// Creates a set-spec-constant-default-value pass from a mapping from spec-ids
// to the default values in the form of spvstd::string.
...
Optimizer::PassToken SPV_API CreateSetSpecConstantDefaultValuePass(
    const spvstd::unordered_map<uint32_t, spvstd::string>& id_value_map);
...
// OpSpecConstant{|True|False} instructions).
Optimizer::PassToken SPV_API CreateSetSpecConstantDefaultValuePass(
    const spvstd::unordered_map<uint32_t, spvstd::vector<uint32_t>>& id_value_map);
...
// it removes.
Optimizer::PassToken SPV_API CreateFlattenDecorationPass();
...
// OpSpecConstantOp.
Optimizer::PassToken SPV_API CreateFreezeSpecConstantValuePass();
...
Optimizer::PassToken SPV_API CreateFoldSpecConstantOpAndCompositePass();
...
Optimizer::PassToken SPV_API CreateUnifyConstantPass();
...
// OpSpecConstantOp.
Optimizer::PassToken SPV_API CreateEliminateDeadConstantPass();
...
Optimizer::PassToken SPV_API CreateStrengthReductionPass();
...
Optimizer::PassToken SPV_API CreateBlockMergePass();
...
Optimizer::PassToken SPV_API CreateInlineExhaustivePass();
...
Optimizer::PassToken SPV_API CreateInlineOpaquePass();
...
Optimizer::PassToken SPV_API CreateLocalSingleBlockLoadStoreElimPass();
...
// possible.
Optimizer::PassToken SPV_API CreateDeadBranchElimPass();
...
Optimizer::PassToken SPV_API CreateLocalMultiStoreElimPass();
...
// possible.
Optimizer::PassToken SPV_API CreateLocalAccessChainConvertPass();
...
Optimizer::PassToken SPV_API CreateLocalSingleStoreElimPass();
...
Optimizer::PassToken SPV_API CreateInsertExtractElimPass();

// Creates a dead insert elimination pass.
// This pass processes each entry point function in the module, searching for
// unreferenced inserts into composite types. These are most often unused
// stores to spvstd::vector components. They are unused because they are never
...
Optimizer::PassToken SPV_API CreateDeadInsertElimPass();
...
// is not allowed.
Optimizer::PassToken SPV_API CreateAggressiveDCEPass();
Optimizer::PassToken SPV_API CreateAggressiveDCEPass(bool preserve_interface);
...
// stages.
Optimizer::PassToken SPV_API CreateRemoveUnusedInterfaceVariablesPass();
...
Optimizer::PassToken SPV_API CreatePropagateLineInfoPass();

// Creates an empty pass.
// This is deprecated and will be removed.
// TODO(jaebaek): remove this pass after handling glslang's broken unit tests.
//                https://github.com/KhronosGroup/glslang/pull/2440
Optimizer::PassToken SPV_API CreateRedundantLineInfoElimPass();

// Creates a compact ids pass.
// The pass remaps result ids to a compact and gapless range starting from %1.
Optimizer::PassToken SPV_API CreateCompactIdsPass();
...
Optimizer::PassToken SPV_API CreateRemoveDuplicatesPass();
...
// elimination.
Optimizer::PassToken SPV_API CreateMergeReturnPass();
...
Optimizer::PassToken SPV_API CreateLocalRedundancyEliminationPass();

// Create LICM pass.
// This pass will look for invariant instructions inside loops and hoist them to
// the loops preheader.
Optimizer::PassToken SPV_API CreateLoopInvariantCodeMotionPass();

// Creates a loop fission pass.
// This pass will split all top level loops whose register pressure exceedes the
// given |threshold|.
Optimizer::PassToken SPV_API CreateLoopFissionPass(size_t threshold);
...
Optimizer::PassToken SPV_API  CreateLoopFusionPass(size_t max_registers_per_loop);
...
Optimizer::PassToken SPV_API CreateLoopPeelingPass();
...
// consider replacing.
Optimizer::PassToken SPV_API
CreateScalarReplacementPass(uint32_t size_limit = 100);
...
Optimizer::PassToken SPV_API CreatePrivateToLocalPass();
...
Optimizer::PassToken SPV_API CreateCCPPass();
...
Optimizer::PassToken SPV_API CreateWorkaround1209Pass();

// Creates a pass that converts if-then-else like assignments into OpSelect.
Optimizer::PassToken SPV_API CreateIfConversionPass();
...
Optimizer::PassToken SPV_API CreateReplaceInvalidOpcodePass();

// Creates a pass that simplifies instructions using the instruction folder.
Optimizer::PassToken SPV_API CreateSimplificationPass();
...
Optimizer::PassToken SPV_API CreateLoopUnrollPass(bool fully_unroll,
                                                  int factor = 0);
...
Optimizer::PassToken SPV_API CreateSSARewritePass();
...
Optimizer::PassToken SPV_API CreateConvertRelaxedToHalfPass();
...
Optimizer::PassToken SPV_API CreateRelaxFloatOpsPass();
...
Optimizer::PassToken SPV_API CreateCopyPropagateArraysPass();

// Create a spvstd::vector dce pass.
// This pass looks for components of vectors that are unused, and removes them
// from the spvstd::vector.  Note this would still leave around lots of dead code that
// a pass of ADCE will be able to remove.
Optimizer::PassToken SPV_API CreateVectorDCEPass();
...
Optimizer::PassToken SPV_API CreateReduceLoadSizePass(
...
Optimizer::PassToken SPV_API CreateCombineAccessChainsPass();
...
Optimizer::PassToken SPV_API CreateInstBindlessCheckPass(
...
// descriptor set |desc_set|. It will write |shader_id| in each output record
// to identify the shader module which generated the record.
Optimizer::PassToken SPV_API CreateInstBuffAddrCheckPass(uint32_t desc_set,
                                                 uint32_t shader_id);

// Create a pass to instrument OpDebugPrintf instructions.
// This pass replaces all OpDebugPrintf instructions with instructions to write
// a record containing the spvstd::string id and the all specified values into a special
...
Optimizer::PassToken SPV_API CreateInstDebugPrintfPass(uint32_t desc_set,
                                               uint32_t shader_id);
...
Optimizer::PassToken SPV_API CreateUpgradeMemoryModelPass();
...
Optimizer::PassToken SPV_API CreateCodeSinkingPass();
...
Optimizer::PassToken SPV_API CreateFixStorageClassPass();
...
//   inclusive.
Optimizer::PassToken SPV_API CreateGraphicsRobustAccessPass();
...
Optimizer::PassToken SPV_API CreateSpreadVolatileSemanticsPass();
...
Optimizer::PassToken SPV_API CreateReplaceDescArrayAccessUsingVarIndexPass();
...
// for the first index.
Optimizer::PassToken SPV_API CreateDescriptorScalarReplacementPass();
...
Optimizer::PassToken SPV_API CreateWrapOpKillPass();
...
// capabilities.
Optimizer::PassToken SPV_API CreateAmdExtToKhrPass();
...
Optimizer::PassToken SPV_API CreateInterpolateFixupPass();
...
// types.
Optimizer::PassToken SPV_API CreateEliminateDeadInputComponentsPass();
...
Optimizer::PassToken SPV_API CreateConvertToSampledImagePass(
    const spvstd::vector<opt::DescriptorSetAndBinding>&
        descriptor_set_binding_pairs);

// Create an interface-variable-scalar-replacement pass that replaces array or
// matrix interface variables with a series of scalar or spvstd::vector interface
// variables. For example, it replaces `float3 foo[2]` with `float3 foo0, foo1`.
Optimizer::PassToken SPV_API CreateInterfaceVariableScalarReplacementPass();
...
Optimizer::PassToken SPV_API CreateRemoveDontInlinePass();
...
// to the function
Optimizer::PassToken SPV_API CreateFixFuncCallArgumentsPass();
}  // namespace spvtools
