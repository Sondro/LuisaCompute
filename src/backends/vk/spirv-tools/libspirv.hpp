...
class SPV_API ValidatorOptions {
 public:
...
class SPV_API OptimizerOptions {
 public:
...
class SPV_API SpirvTools {
 public:
  ...
  bool Assemble(const spvstd::string& text, spvstd::vector<uint32_t>* binary,
  ...
  bool Disassemble(const spvstd::vector<uint32_t>& binary, spvstd::string* text,
  ...
  // binary itself.
  bool Validate(const spvstd::vector<uint32_t>& binary) const;
