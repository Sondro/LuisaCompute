...
public:
	...
	void visit(AccelBuildCommand const* cmd) override {
		...
		auto& updateInfo = stream->updateInfos.emplace_back(
			accel->Preprocess(cb, stream->StateTracker(), cmd->instance_count(), cmd->request() == AccelBuildRequest::PREFER_UPDATE, mod.size(), frameRes));
		if (!mod.empty()) {
			...
			frameRes->AddCopyCmd(
				...
				[&] -> vstd::optional<VkBufferCopy> {
					...
					r.New(accel->SetInstance(
						updateInfo,
						v.index,
						reinterpret_cast<Mesh const*>(v.mesh),
						v.affine,
						uploadBuffer.buffer,
						uploadOffset,
	...
	void operator()(ShaderDispatchCommand::AccelArgument const& bf) {
		auto accel = reinterpret_cast<Accel const*>(bf.handle)->AccelBuffer();
...
void LCStream::DispatchCmd(CommandList&& cmdList) {
	...
	for (auto&& list : cmdLists) {
		...
		if (requireSync || !compactAccel.empty())
			StateTracker().Execute(cmdBuffer);
