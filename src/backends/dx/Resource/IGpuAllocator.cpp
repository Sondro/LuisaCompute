...
public:
  ...
  uint64 AllocateTextureHeap(Device *device, GFXFormat format, uint32_t width,
                             uint32_t height, uint32_t depthSlice,
                             TextureDimension dimension, uint32_t mipCount,
                             ID3D12Heap **heap, uint64_t *offset,
                             bool isRenderTexture) override {
    using namespace D3D12MA;
    D3D12_HEAP_FLAGS heapFlag =
        isRenderTexture ? D3D12_HEAP_FLAG_ALLOW_ONLY_RT_DS_TEXTURES
    ...
    info.SizeInBytes = Resource::GetTextureSize(
        device, width, height, format, dimension, depthSlice, mipCount);
    Allocation *alloc;
  ...
  uint64 AllocateBufferHeap(Device *device, uint64_t targetSizeInBytes,
  ...
  DefaultAllocator(Device *device) {
    using namespace D3D12MA;
    ALLOCATOR_DESC desc;
...
IGpuAllocator *IGpuAllocator::CreateAllocator(Device *device, Tag tag) {
  switch (tag) {
