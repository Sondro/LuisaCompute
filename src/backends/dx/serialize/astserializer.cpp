...
vstd::unique_ptr<IJsonDict> AstSerializer::Serialize(MemberExpr const& t, IJsonDatabase* db) {
	...
	r->Set("swizzle_size", (int64)t._swizzle_size);
	r->Set("swizzle_code", (int64)t._swizzle_code);
	return r;
}
void AstSerializer::DeSerialize(MemberExpr& t, IJsonDict* r, DeserVisitor const& evt) {
	...
	t._swizzle_size = ReadVar::get_or<int64>(r->Get("swizzle_size"), 0);
	t._swizzle_code = ReadVar::get_or<int64>(r->Get("swizzle_code"), 0);
}
...
void DeserVisitor::GetExpr(vstd::function<void(Expression*)> const& func) const {
	for (auto&& i : expr) {
		func(i.second.second);
		i.second.first = nullptr;
	}
}
void DeserVisitor::GetStmt(vstd::function<void(Statement*)> const& func) const {
	for (auto&& i : stmt) {
		func(i.second.second);
		i.second.first = nullptr;
	}
