...
void AstSerializer::SerExprDerive(ConstantExpr const* expr, IJsonDict* dict) {
	dict->Set("value"sv, expr->data().hash());
}
template<>
...
vstd::unique_ptr<IJsonDict> AstSerializer::SerNewFunction(Function func) {
	...
	//////////////////// Constant
	{
		...
		for (auto&& i : consts) {
			...
			Serialize(i.second.data, localDict.get());
			localDict->Set("consttype"sv, SerType(i.second.type));
			constDict->Set(i.first, std::move(localDict));
		}
